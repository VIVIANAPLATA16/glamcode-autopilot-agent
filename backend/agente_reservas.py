...
def procesar_mensaje(mensaje: str) -> dict[str, Any]:
    ...
    # Baja confianza → escalamiento, nunca improvisar
    ...
    if intencion == "gestion_cita":
        return _herramienta_gestionar_cita(mensaje, clasificacion)

    if intencion == "cotizacion":
        return _herramienta_cotizar_servicios(mensaje, clasificacion)
...
def _clasificar_intencion(mensaje: str) -> dict[str, Any]:
...
Analiza el mensaje del cliente y responde SOLO con JSON válido con esta estructura:
{
...
- "cotizacion": preguntas de precio, costos, cuánto cuesta
...
def _manejar_queja(mensaje: str, clasificacion: dict[str, Any]) -> dict[str, Any]:
    ...
    conv_id = rh.guardar_conversacion(
        mensaje_cliente=mensaje,
        intencion="queja",
        herramienta="compuerta_queja",
        respuesta_agente=MENSAJE_QUEJA_CLIENTE,
        escalado_revision=True,
...
def _herramienta_gestionar_cita(
    mensaje: str, clasificacion: dict[str, Any]
) -> dict[str, Any]:
    """
    ...
    """
    extraccion = _extraer_datos_cita(mensaje)
    ...
    # Resolver servicio_id desde texto libre
    servicio_id = _resolver_servicio_id(servicio_solicitado) if servicio_solicitado else None

    # Validaciones que requieren revisión humana
...
def _extraer_datos_cita(mensaje: str) -> dict[str, Any]:
...
def _herramienta_cotizar_servicios(
    ...
    servicios_mencionados = _extraer_servicios_cotizacion(mensaje)
    ...
    for nombre in servicios_mencionados:
        servicio_id = _resolver_servicio_id(nombre)
    ...
    if desconocidos:
        return _escalar_revision(
            mensaje=mensaje,
            intencion="cotizacion",
            motivo=f"Servicios no encontrados en catálogo: {', '.join(desconocidos)}",
...
def _extraer_servicios_cotizacion(mensaje: str) -> list[str]:
    ...
    system_prompt = f"""Identifica qué servicios del catálogo menciona el cliente para cotización.
Catálogo: {catalogo}

Responde SOLO JSON:
{{"servicios": ["nombre o alias mencionado", ...]}}
...
def _resolver_servicio_id(texto: str | None) -> str | None:
    if not texto:
        return None
    texto_lower = texto.lower().strip()
    for sid, info in SERVICIOS.items():
        if texto_lower in sid or texto_lower == info["nombre"].lower():
            return sid
        for alias in info["aliases"]:
            if alias.lower() in texto_lower or texto_lower in alias.lower():
                return sid
    return None
...
def _escalar_revision(
    *,
    ...
    conv_id = rh.guardar_conversacion(
        ...
        respuesta_agente=MENSAJE_ESCALAMIENTO,
        escalado_revision=True,
        revision_id=revision["id"],
    )
